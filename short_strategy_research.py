...
def run_eurjpy_final_validation():
    try:
        EV_STATUS.update(
            state="starting",
            message="Starting EUR/JPY final validation + current20 comparison",
            progress=1,
        )

        # ----------------------------------------------------
        # 1) Exact current 20 baseline
        # ----------------------------------------------------
        baseline = ev_rebuild_current20()

        write_csv(
            EV_OUT["baseline_parity"],
            baseline["parity"],
        )
        ...
        write_csv(
            EV_OUT["baseline_summary"],
            baseline_summary_rows,
        )

        # ----------------------------------------------------
        # 2) EUR/JPY exact targeted candidates
        # ----------------------------------------------------
        EV_STATUS.update(
            state="eurjpy_fetch",
            message="Fetching EUR/JPY H1 history",
            progress=62,
        )

        eurjpy, _ = fetch_history(
            EV_PAIR,
            "H1",
            START,
            NOW,
        ...
        if len(eurjpy) < 90000:
            raise RuntimeError(
                f"Incomplete EUR/JPY H1 history: {len(eurjpy)} candles"
            )

        atr_values = ev_atr14(eurjpy)
        ...
        total_candidates = len(EV_LOOKBACKS) * len(EV_VARIANTS)
        done = 0

        for lookback in EV_LOOKBACKS:
            for variant in EV_VARIANTS:
                ...
                EV_STATUS.update(
                    ...
                    message=(
                        ...
                        f"LB{lookback} {variant['variant_id']}"
                    ),
                    progress=64 + int(12 * done / total_candidates),
                )

                trades = ev_build_candidate_trades(
                    eurjpy,
                    atr_values,
                    lookback,
                    variant,
                ...
                candidate_cache[(lookback, variant["variant_id"])] = trades

                reference = EV_REFERENCE_COUNTS[
                    (lookback, variant["variant_id"])
                ]

                if len(trades) < reference:
                    raise RuntimeError(
                        f"EUR/JPY parity failure LB{lookback} "
                        f"{variant['variant_id']}: "
                        f"{len(trades)} < prior {reference}"
                    )

                summary = ev_candidate_summary_row(
                    lookback,
                    variant["variant_id"],
                    trades,
                )
                ...
                for multiplier in (0.5, 1.0, 1.5, 2.0):
                    stressed = ev_build_candidate_trades(
                        eurjpy,
                        atr_values,
                        lookback,
                        variant,
                    ...
                    candidate_cost_stress.append({
                        ...
                        "variant_id": variant["variant_id"],
                        "cost_multiplier": multiplier,
                        "adverse_cost_ticks": EV_BASE_COST_TICKS * multiplier,
                        "adverse_cost_pips": (
                            EV_BASE_COST_TICKS
                            * EV_TICK
                            / EV_PIP
                ...
                for trade in trades:
                    candidate_trade_rows.append(
                        ev_serialise_trade(trade)
                    )

        write_csv(EV_OUT["candidate_summary"], candidate_summary)
        write_csv(EV_OUT["candidate_periods"], candidate_periods)
        write_csv(EV_OUT["candidate_cost_stress"], candidate_cost_stress)
        write_csv(EV_OUT["candidate_calendar"], candidate_calendar)
        write_csv(
            EV_OUT["candidate_calendar_summary"],
            ev_candidate_calendar_summary(candidate_calendar),
        )
        write_csv(EV_OUT["candidate_rolling"], candidate_rolling)
        write_csv(
            EV_OUT["candidate_rolling_summary"],
            ev_candidate_rolling_summary(candidate_rolling),
        )
        write_csv(EV_OUT["candidate_trades"], candidate_trade_rows)

        # ----------------------------------------------------
        # 3) Add each candidate ONE AT A TIME to current 20
        # ----------------------------------------------------
        ...
        # Include baseline first for direct CSV comparison.
        ...
        for lookback in EV_LOOKBACKS:
            for variant in EV_VARIANTS:
                ...
                candidate_id = (
                    f"EURJPY_LB{lookback}_{variant['variant_id']}"
                )

                EV_STATUS.update(
                    state="portfolio_addition",
                    message=f"Testing {candidate_id} against current 20",
                    progress=77 + int(17 * candidate_no / total_candidates),
                )

                candidate_trades = candidate_cache[
                    (lookback, variant["variant_id"])
                ]
                ...
                for priority, mode in [
                    ...
                    # EUR/JPY is a new pair, so it should never be rejected
                    # because of the existing 20-strategy portfolio.
                    eurjpy_rejections = [
                        x for x in rejected
                        if x["candidate_pair"] == EV_PAIR
                    ]
                    ...
                    sim = row.pop("_sim")
                    row["eurjpy_candidate_trades_before_gate"] = len(
                        candidate_trades
                    )
                    row["eurjpy_candidate_gate_rejections"] = len(
                        eurjpy_rejections
                    )
        ...
        write_csv(
            EV_OUT["portfolio_summary"],
            portfolio_summary_rows,
        )
        write_csv(
            EV_OUT["portfolio_delta"],
            portfolio_delta_rows,
        )
        write_csv(
            EV_OUT["portfolio_frequency"],
            portfolio_frequency_rows,
        )
        write_csv(
            EV_OUT["portfolio_rolling"],
            portfolio_rolling_rows,
        )
        write_csv(
            EV_OUT["portfolio_rolling_summary"],
            ev_portfolio_rolling_summary(portfolio_rolling_rows),
        )
        write_csv(
            EV_OUT["portfolio_calendar"],
            portfolio_calendar_rows,
        )
        write_csv(
            EV_OUT["portfolio_calendar_summary"],
            ev_portfolio_calendar_summary(portfolio_calendar_rows),
        )
        write_csv(
            EV_OUT["portfolio_gate_rejections"],
            gate_rejections,
        )
        ...
        notes = [
            {
                "item": "scope",
                "value": (
                    "Final targeted EUR/JPY H1 LONG validation only: "
                    "lookbacks 12/15/20 crossed with CONTROL, exclude Friday, "
                    "exclude NY08, and exclude Friday+NY08."
                ),
            },
            {
                "item": "frozen_entry_geometry",
                "value": (
                    "Exact bullish engulf; BR>=1.30; body>=1.25 ATR14; "
                    "abs(signal low - prior rolling low)<=0.15 ATR14; RR5.50; "
                    "stop signal low-10 ticks."
                ),
            },
            {
                ...
                "value": (
                    "EUR/JPY H1 baseline adverse fill is signal close +5 ticks "
                    "= +0.005 = 0.5 pip. 0.5x/1.5x/2x cost stress is exported."
                ),
            },
            {
                "item": "context_clock",
                "value": (
                    "Friday and 08:00 exclusion use America/New_York signal-candle "
                    "OPEN time with DST."
                ),
            },
            {
                "item": "baseline_authority",
                "value": (
                    "The current 20-strategy baseline is rebuilt from the exact "
                    "frozen H1/M15 strategy machinery in this file; no headline "
                    "portfolio figures are hard-coded."
                ),
            },
            {
                "item": "live_account_constraint",
                "value": (
                    "Portfolio additions use LIVE_SAFE non-hedging logic: "
                    "same-direction same-pair overlap allowed; new opposite-direction "
                    "same-pair entry blocked. EUR/JPY is new to the portfolio, so "
                    "candidate trades should have zero blockers from the existing "
                    "20 strategies."
                ),
            },
            {
                "item": "risk",
                "value": (
                    "Portfolio equity simulations use 1% of then-realised equity "
                    "for each accepted H1 or M15 entry, matching the current research "
                    "convention. Live OANDA executor sizes from NAV."
                ),
            },
            {
                "item": "decision_rule",
                "value": (
                    "Prefer the candidate that improves current-20 CAGR / total R "
                    "without materially worsening conservative DD, while preserving "
                    "strong temporal splits, cost stress, rolling windows and a "
                    "reasonable trade count. Do not select by headline PF alone."
                ),
            },
        ]

        write_csv(EV_OUT["notes"], notes)

        EV_STATUS.update(
            state="packaging",
            message="Packaging EUR/JPY final validation results",
            progress=97,
        )
        ...
        with zipfile.ZipFile(
            ...
            for path in EV_OUT.values():
                if os.path.exists(path):
                    z.write(path, arcname=os.path.basename(path))

        EV_STATUS.update(
            state="complete",
            message="EUR/JPY final validation + portfolio-addition test complete",
            progress=100,
            results=EV_BUNDLE,
            current20_live_safe_h1_first_trades=len(
                baseline["live_h1_first"]
            ),
            eurjpy_candidates=total_candidates,
        )
        ...
        EV_STATUS.update(
            state="error",
            message=str(error),
            progress=EV_STATUS.get("progress", 0),
        )

        print(
            "EURJPY FINAL VALIDATION ERROR:",
            repr(error),
            flush=True,
        )
...
@app.route("/eurjpy-final-validation/status")
def eurjpy_final_validation_status():
    return jsonify(EV_STATUS)


@app.route("/eurjpy-final-validation/results")
def eurjpy_final_validation_results():
    if not os.path.exists(EV_BUNDLE):
        return jsonify({
            "status": "not_ready",
            "state": EV_STATUS.get("state"),
            "message": EV_STATUS.get("message"),
        }), 404

    return send_file(
        os.path.abspath(EV_BUNDLE),
        as_attachment=True,
        download_name=EV_BUNDLE,
    )


@app.route("/eurjpy-final-validation/info")
def eurjpy_final_validation_info():
    return jsonify({
        "service": "EUR/JPY H1 LONG final validation + current20 portfolio addition",
        "status": EV_STATUS.get("state"),
        "read_only": True,
        "orders_supported": False,
        "raw_family": {
            "pair": EV_PAIR,
            ...
            "body_ratio_min": EV_BR_MIN,
            "body_atr_min": EV_BODY_ATR_MIN,
            "distance_atr_max": EV_DISTANCE_ATR_MAX,
            "lookbacks": EV_LOOKBACKS,
            "rr": EV_RR,
            "stop_buffer_ticks": EV_STOP_BUFFER_TICKS,
            "base_adverse_ticks": EV_BASE_COST_TICKS,
        },
        "variants": [
            x["variant_id"] for x in EV_VARIANTS
        ],
        "portfolio_baseline": "exact current locked 20 strategies",
...
if __name__ == "__main__":
    threading.Thread(
        target=run_eurjpy_final_validation,
        daemon=True,
    ).start()

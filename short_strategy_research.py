...
STRATEGIES = {
    ...
    "EUR_USD": {
        ...
        # Final replacement long - Candidate A.
        # Bullish engulfing body ratio >= 1.00.
        ...
        # Previous 15 H1 bars excluding current;
        # signal low within 0.10 ATR14 of previous lowest low.
        ...
        # No daily EMA regime or alignment.
        # EMA1 placeholders preserve the shared daily-state builder.
        "fast_daily_ema": 1,
        "slow_daily_ema": 1,
    ...
    # ==============================================
    # USD/JPY - FINAL REPLACEMENT LONG
    # ==============================================
    ...
    "USD_JPY": {
        ...
        # Exact bullish engulfing body ratio >= 1.00.
        "minimum_body_ratio": 1.00,
        ...
        # Signal body >= 0.80 ATR14.
        "minimum_body_atr": 0.80,

        # Previous 100 H1 bars excluding current;
        # signal low within 0.55 ATR14 of previous lowest low.
    ...
    # ==============================================
    # USD/CAD - FINAL REPLACEMENT LONG (CANDIDATE B)
    # ==============================================
    ...
    "USD_CAD": {
        ...
        # Bullish engulfing body ratio >= 1.10
        ...
        # Previous completed Daily ATR14 / 50-day mean >= 0.95
    ...
    # ==============================================
    # EUR/GBP - FINAL REPLACEMENT LONG (CANDIDATE B)
    # ==============================================
    ...
    "EUR_GBP": {
        ...
        # Signal body >= 1.10 ATR14.
        "minimum_body_atr": 1.10,

        # Previous 30 H1 bars excluding current;
        # signal low within 0.075 ATR14 of previous lowest low.
        "structure_lookback": 30,
        "maximum_distance_atr": 0.075,

        # Signal range >= 1.40 ATR14.
        "minimum_range_enabled": True,
        "minimum_range_atr": 1.40,

        # Candidate B uses two separate daily conditions:
        # previous completed daily close > EMA200
        # AND previous completed daily EMA20 > EMA150.
        "daily_close_ema": 200,
        "fast_daily_ema": 20,
        "slow_daily_ema": 150,

        # Disable the old shared close>slow test because
        # the close regime is handled by daily_close_ema.
        "require_daily_close_above_slow": False,
        "require_daily_fast_above_slow": True,
...
def strategy_config(
    ...
    if instrument not in STRATEGIES:
        ...
        return jsonify({
            ...
            "message":
                f"{instrument} has no long strategy"
        }), 400
...
# ============================================================
# FINAL LIVE 10-STRATEGY PORTFOLIO - DEEP STATISTICS
#
# READ-ONLY RESEARCH. NEVER SENDS ORDERS.
#
# Uses the exact STRATEGIES and SHORT_STRATEGIES definitions
# above, i.e. the final currently-deployed live portfolio.
#
# History:
...
# Reports:
#   - full history
#   - trailing 1Y / 2Y / 5Y / 10Y
#   - every calendar year
#   - rolling 2Y windows
#   - rolling 3Y windows
#   - best / worst calendar year
#   - best / worst rolling 2Y
#   - best / worst rolling 3Y
#   - per-strategy summary
#
# Portfolio compounding:
#   £100 start
#   1% realised balance risk at ENTRY
#   overlapping positions retain their fixed entry risk
...
PORTFOLIO_RESEARCH_FROM = datetime(
    2002, 5, 6, 20, 0,
    tzinfo=timezone.utc,
)
...
OUTPUT_OVERVIEW = "final_portfolio_stats_overview.csv"
OUTPUT_CALENDAR = "final_portfolio_stats_calendar_years.csv"
OUTPUT_ROLLING_2Y = "final_portfolio_stats_rolling_2y.csv"
OUTPUT_ROLLING_3Y = "final_portfolio_stats_rolling_3y.csv"
OUTPUT_EXTREMES = "final_portfolio_stats_best_worst.csv"
OUTPUT_STRATEGIES = "final_portfolio_stats_by_strategy.csv"
OUTPUT_TRADES = "final_portfolio_stats_full_trade_log.csv"

DEEP_STATS_STATUS = {
    "state": "not_started",
    "message": "Portfolio analysis has not started",
    "service": "Final Live Portfolio Deep Statistics",
...
def normalise_trade_times(trades):
    result = []

    for trade in trades:
        row = dict(trade)

        row["entry_dt"] = portfolio_parse_iso(
            row["entry_time_utc"]
        )

        row["exit_dt"] = portfolio_parse_iso(
            row["exit_time_utc"]
        )
...
def build_calendar_years(
    ...
    for year in range(
        PORTFOLIO_RESEARCH_FROM.year,
        PORTFOLIO_RESEARCH_TO.year + 1,
    ):
        start = max(
            PORTFOLIO_RESEARCH_FROM,
            datetime(
...
def build_rolling_year_windows(
    trades,
    years,
):
    rows = []
    ...
    for start_year in range(
        PORTFOLIO_RESEARCH_FROM.year,
        PORTFOLIO_RESEARCH_TO.year - years + 2,
    ):
        start = max(
            PORTFOLIO_RESEARCH_FROM,
            datetime(
        ...
        # Only include complete rolling windows.
        if (
            end - start
        ).total_seconds() < (
            365.0
...
def run_deep_portfolio_stats():
    try:
        DEEP_STATS_STATUS.update({
            "state":
                "running",
            "message":
                "Running full-history final live portfolio backtest",
            "from":
                iso_utc(
                    PORTFOLIO_RESEARCH_FROM
                ),
        ...
        DEEP_STATS_STATUS.update({
            "state":
                "complete",
            "message":
                "Final live portfolio deep statistics complete",
            "closed_trades":
                len(
                    trades
                ),
        ...
        print(
            "FINAL LIVE PORTFOLIO DEEP STATS COMPLETE"
        )
        print(
            "=" * 100
        )
...
@app.route(
    "/portfolio-stats/status"
)
def portfolio_stats_status():
    return jsonify(
        DEEP_STATS_STATUS
    )
...
@app.route(
    "/portfolio-stats/calendar"
)
def download_stats_calendar():
    return deep_send_file(
        OUTPUT_CALENDAR
    )
...
@app.route(
    "/portfolio-stats/rolling-2y"
)
def download_stats_rolling_2y():
    return deep_send_file(
        OUTPUT_ROLLING_2Y
    )
...
@app.route(
    "/portfolio-stats/rolling-3y"
)
def download_stats_rolling_3y():
    return deep_send_file(
        OUTPUT_ROLLING_3Y
    )
...
@app.route(
    "/portfolio-stats/extremes"
)
def download_stats_extremes():
    return deep_send_file(
        OUTPUT_EXTREMES
    )
...
@app.route(
    "/portfolio-stats/strategies"
)
def download_stats_strategies():
    return deep_send_file(
        OUTPUT_STRATEGIES
    )
...
@app.route(
    "/portfolio-stats/trades"
)
def download_stats_trades():
    return deep_send_file(
        OUTPUT_TRADES
    )

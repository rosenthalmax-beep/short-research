...
# ============================================================
# EUR/JPY M15 SHORT #24 — 0.10 ATR SWEEP-DEPTH CONFIRMATION
# + EXACT 23 -> 24 PORTFOLIO ADD AT FULL 1% RISK
# ============================================================
#
# THIS IS NOT A NEW OPTIMISATION.
#
# PREDECLARED CANDIDATE:
#   Trigger A minimum sweep depth = 0.10 * M15 ATR14
#
# NEIGHBOURS ARE DIAGNOSTIC ONLY:
#   0.075 / 0.125 / 0.150 ATR
#
# Context anchors are also reproduced:
#   no minimum / 0.05 / 0.20 ATR
#
# The runner MUST NOT replace 0.10 with a better-looking neighbour.
#
# Trigger B remains frozen.
# A/B RRs remain frozen at 4.75 / 3.00.
# B retains same-candle priority.
# Stop remains signal high +10 ticks.
# Historical base fill remains 1 pip adverse.
# p0 / half-open overlap is unchanged.
#
# After standalone stability confirmation, ONLY the fixed 0.10 candidate is
# run through the exact current23 -> current24 live-safe non-hedging gate at
# full 1.00% risk.
#
# READ ONLY. NEVER SENDS ORDERS.
# ============================================================
...
SD24_STATUS = {
    "state": "not_started",
    "message": "EURJPY M15 SHORT #24 sweep-depth confirmation not started",
    "progress": 0,
    "orders_supported": False,
    "trading_enabled": False,
}

SD24_CANDIDATE_DEPTH = 0.10
SD24_DIAGNOSTIC_DEPTHS = [0.075, 0.125, 0.150]
SD24_CONTEXT_DEPTHS = [0.0, 0.05, 0.20]
SD24_ALL_DEPTHS = [0.0, 0.05, 0.075, 0.10, 0.125, 0.150, 0.20]
SD24_COSTS = [1.0, 2.0, 3.0]

SD24_BUNDLE = "EURJPY_M15_SHORT_24_SWEEP_DEPTH_CONFIRMATION_AND_PORTFOLIO_ADD_RESULTS.zip"

SD24_OUT = {
    "baseline_parity": "eurjpy_m15_short_24_sweep_confirm_baseline_parity.csv",
    "depth_manifest": "eurjpy_m15_short_24_sweep_confirm_depth_manifest.csv",
    "standalone_summary": "eurjpy_m15_short_24_sweep_confirm_standalone_summary.csv",
    "standalone_periods": "eurjpy_m15_short_24_sweep_confirm_standalone_periods.csv",
    "cost_stress": "eurjpy_m15_short_24_sweep_confirm_cost_stress.csv",
    "rolling": "eurjpy_m15_short_24_sweep_confirm_rolling.csv",
    "rolling_summary": "eurjpy_m15_short_24_sweep_confirm_rolling_summary.csv",
    "calendar": "eurjpy_m15_short_24_sweep_confirm_calendar.csv",
    "calendar_summary": "eurjpy_m15_short_24_sweep_confirm_calendar_summary.csv",
    "candidate_neighbour_check": "eurjpy_m15_short_24_sweep_confirm_candidate_neighbour_check.csv",
    "portfolio_summary": "eurjpy_m15_short_24_sweep_confirm_portfolio_summary.csv",
    "portfolio_delta": "eurjpy_m15_short_24_sweep_confirm_portfolio_delta.csv",
    "portfolio_gate_summary": "eurjpy_m15_short_24_sweep_confirm_portfolio_gate_summary.csv",
    "portfolio_gate_displacements": "eurjpy_m15_short_24_sweep_confirm_portfolio_gate_displacements.csv",
    "portfolio_rolling": "eurjpy_m15_short_24_sweep_confirm_portfolio_rolling.csv",
    "portfolio_rolling_summary": "eurjpy_m15_short_24_sweep_confirm_portfolio_rolling_summary.csv",
    "portfolio_calendar": "eurjpy_m15_short_24_sweep_confirm_portfolio_calendar.csv",
    "portfolio_calendar_summary": "eurjpy_m15_short_24_sweep_confirm_portfolio_calendar_summary.csv",
    "portfolio_periods": "eurjpy_m15_short_24_sweep_confirm_portfolio_periods.csv",
    "notes": "eurjpy_m15_short_24_sweep_confirm_notes.csv",
}
...
# ============================================================
# SWEEP-DEPTH FEATURE / EXECUTION HELPERS
# ============================================================

def sd24_sweep_depth_array(m15, qfeatures):
    n = len(m15)
    ...
    depth[valid] = (high[valid] - prior40[valid]) / atr_[valid]
    return depth


def sd24_a_indices_at_depth(base_a_indices, sweep_depth, threshold):
    threshold = float(threshold)

    if threshold <= 0:
        return list(base_a_indices)

    return [
        i for i in base_a_indices
        if np.isfinite(sweep_depth[i])
        and sweep_depth[i] >= threshold
    ]


def sd24_short_outcome(m15, signal_index, rr, cost_pips=1.0):
    """
    Exact frozen #24 short outcome convention, except historical adverse cost
    can be stress-tested at 1/2/3 pips.
    """
    signal = m15[signal_index]
    ...
    for j in range(signal_index + 1, len(m15)):
        ...
        if hit_stop and hit_target:
            # Exact frozen SHORT ambiguity convention.
            if (
                abs(float(bar["open"]) - float(bar["low"]))
                < abs(float(bar["high"]) - float(bar["open"]))
            ):
...
def sd24_build_trades(
    m15,
    a_indices,
    b_indices,
    cost_pips=1.0,
    candidate_id="EURJPY_M15_SHORT_24_SWEEP_DEPTH_CONFIRM",
):
    """
    Exact frozen A+B / B-priority / p0 union using supplied Trigger A indices.
    B remains frozen.
    """
    events = (
        [
    ...
    while p < len(events):
        ...
        # Half-open overlap [signal_index, exit_index).
        # Signal exactly on exit candle remains eligible.
        p = q
        while p < len(events) and events[p][0] < exit_index:
            p += 1
...
def sd24_standalone_period_rows(depth, trades):
    rows = []

    for label, start, end in sd24_period_defs():
        s = calc_stats(subset(trades, start, end))
        rows.append({
            "sweep_depth_atr": depth,
            "candidate_fixed_0p10": abs(depth - SD24_CANDIDATE_DEPTH) < 1e-12,
            "period": label,
            **s,
        })
...
def sd24_standalone_summary(period_rows):
    grouped = defaultdict(dict)

    for row in period_rows:
        grouped[row["sweep_depth_atr"]][row["period"]] = row
    ...
    for depth in sorted(grouped):
        ...
        out.append({
            ...
            "role": (
                ...
                if abs(depth - 0.10) < 1e-12
                else "DIAGNOSTIC_NEIGHBOUR"
                if depth in SD24_DIAGNOSTIC_DEPTHS
                else "CONTEXT_ANCHOR"
            ),
            ...
            "era2020_trades": p["ERA_2020_PLUS"]["trades"],
...
def sd24_month_floor(dt):
    return datetime(dt.year, dt.month, 1, tzinfo=timezone.utc)


def sd24_add_months(dt, months):
    total = dt.year * 12 + (dt.month - 1) + int(months)
    year, month0 = divmod(total, 12)
    return datetime(year, month0 + 1, 1, tzinfo=timezone.utc)


def sd24_rolling_rows(depth, trades):
    if not trades:
        return []

    first = sd24_month_floor(min(t["entry_time"] for t in trades))
    end_complete = sd24_month_floor(NOW)

    rows = []

    for months in (12, 24, 36):
        cur = first
        while sd24_add_months(cur, months) <= end_complete:
            end = sd24_add_months(cur, months)
            s = calc_stats(subset(trades, cur, end))
            ...
            cur = sd24_add_months(cur, 1)

    return rows


def sd24_rolling_summary(rows):
    grouped = defaultdict(list)
    ...
    for (depth, months), group in grouped.items():
        active = [x for x in group if x["trades"] > 0]
...
def sd24_calendar_rows(depth, trades):
    if not trades:
        return []
...
def sd24_calendar_summary(rows):
    grouped = defaultdict(list)

    for row in rows:
        grouped[row["sweep_depth_atr"]].append(row)
...
def sd24_cost_rows(depth, m15, a_indices, b_indices):
    rows = []

    for cost in SD24_COSTS:
        trades = sd24_build_trades(
            m15,
            a_indices,
            b_indices,
            cost_pips=cost,
            candidate_id=f"EURJPY_M15_SHORT_DEPTH_{depth:.3f}_COST_{cost:.1f}",
        )
...
def sd24_neighbour_check(summary_rows, rolling_summary, calendar_summary):
    """
    0.10 is fixed ex ante.
    This file measures whether adjacent settings tell the same broad story;
    it never ranks or replaces the candidate.
    """
    s = {round(x["sweep_depth_atr"], 3): x for x in summary_rows}
    r = {
        (round(x["sweep_depth_atr"], 3), int(x["months"])): x
        for x in rolling_summary
    }
    c = {round(x["sweep_depth_atr"], 3): x for x in calendar_summary}

    candidate = s[0.100]

    rows = []

    for depth in [0.075, 0.100, 0.125, 0.150]:
        row = s[round(depth, 3)]
        r24 = r[(round(depth, 3), 24)]
        r36 = r[(round(depth, 3), 36)]
        cal = c[round(depth, 3)]
        ...
        rows.append({
            ...
            "role": (
                "PREDECLARED_CANDIDATE"
                if abs(depth - 0.10) < 1e-12
                else "DIAGNOSTIC_NEIGHBOUR_ONLY"
            ),
            "must_not_replace_candidate_based_on_rank": True,
            "full_trades": row["full_trades"],
            "full_pf": row["full_pf"],
            "full_r": row["full_r"],
            "full_dd_r": row["full_dd_r"],
            "validation2018_pf": row["validation2018_pf"],
            "validation2018_r": row["validation2018_r"],
            "last5y_pf": row["last5y_pf"],
            "last5y_r": row["last5y_r"],
            "last3y_pf": row["last3y_pf"],
            "last3y_r": row["last3y_r"],
            ...
            "positive_active_years_pct": cal[
                "positive_active_years_pct"
            ],
            "delta_pf_vs_fixed_0p10": (
                row["full_pf"] - candidate["full_pf"]
            ),
            "delta_dd_r_vs_fixed_0p10": (
                row["full_dd_r"] - candidate["full_dd_r"]
            ),
        })
...
def sd24_portfolio_period_row(label, mode, variant, sim, trades, start, end):
    def bal_before(ts):
        j = bisect.bisect_left(sim["exit_times"], ts) - 1
        return sim["exit_balances"][j] if j >= 0 else STARTING_BALANCE
...
def sd24_portfolio_rolling_rows(mode, variant, sim, trades):
    first = sd24_month_floor(min(t["entry_time"] for t in trades))
    end_complete = sd24_month_floor(NOW)
...
def sd24_portfolio_rolling_summary(rows):
    grouped = defaultdict(list)
    ...
    for (mode, variant, months), group in grouped.items():
        vals = [x["compounded_return_pct"] for x in group]
        positives = [x for x in group if x["compounded_return_pct"] > 0]
...
def sd24_portfolio_calendar_rows(mode, variant, sim, trades):
    first_year = min(t["entry_time"] for t in trades).year
    rows = []
...
def sd24_portfolio_calendar_summary(rows):
    grouped = defaultdict(list)
    ...
    for (mode, variant), group in grouped.items():
        active = [x for x in group if x["realized_exits"] > 0]
        positives = [x for x in active if x["compounded_return_pct"] > 0]
        vals = [x["compounded_return_pct"] for x in active]
...
def sd24_portfolio_summary_row(mode, variant, trades, sim):
    s = sim["summary"]
    ...
    return {
        "portfolio_mode": mode,
        "variant": variant,
        "accepted_portfolio_trades": len(trades),
        "accepted_q24_trades": len(candidate_trades),
        "candidate_risk_pct": (
            0.0 if variant == "CURRENT23_BASELINE"
            else 1.0
        ),
        "historical_cagr_pct": s["historical_cagr_pct"],
        "ending_multiple": s["ending_multiple"],
...
def sd24_displacement_rows(mode, baseline_trades, candidate_trades, rejected):
    baseline_ids = {
        (t["strategy_id"], t["entry_time"]): t
        for t in baseline_trades
    }
    candidate_ids = {
        (t["strategy_id"], t["entry_time"]): t
    ...
    # Existing current23 trades displaced by adding #24.
    for key, t in baseline_ids.items():
        if key not in candidate_ids:
            rows.append({
                "portfolio_mode": mode,
                "event": "EXISTING_TRADE_DISPLACED",
                "strategy_id": t["strategy_id"],
                "pair": t["pair"],
                "side": t["side"],
                "entry_time_utc": iso(t["entry_time"]),
                "r": t["r"],
                "reason": "",
            })

    # Candidate rejections from live-safe gate.
    for r in rejected:
        if r.get("candidate_strategy_id") != Q24_STRATEGY_ID:
            continue

        rows.append({
            "portfolio_mode": mode,
            "event": "Q24_REJECTED_BY_GATE",
            "strategy_id": Q24_STRATEGY_ID,
            "pair": Q24_PAIR,
            "side": "SELL",
...
def run_sd24_confirmation():
    try:
        global EV_STATUS
        EV_STATUS = SD24_STATUS
        ...
        SD24_STATUS.update(
            state="rebuild",
            message="Rebuilding exact frozen #24 baseline and current23",
            progress=9,
        )

        qfeatures = q24_features(
            eurjpy_m15,
            eurjpy_h1,
        )

        base_a = qfeatures["a_indices"]
        base_b = qfeatures["b_indices"]
        sweep_depth = sd24_sweep_depth_array(
            eurjpy_m15,
            qfeatures,
        )

        baseline_trades = q24_build_candidate_trades(
            eurjpy_m15,
            qfeatures,
        )
        ...
        parity = {
            "status": "PASS",
            "reference_trades": SD24_BASELINE_REF["trades"],
            "current_trades": baseline_stats["trades"],
            "reference_pf": SD24_BASELINE_REF["pf"],
            "current_pf": baseline_stats["profit_factor"],
            "reference_r": SD24_BASELINE_REF["r"],
            "current_r": baseline_stats["total_r"],
            "reference_dd_r": SD24_BASELINE_REF["dd_r"],
            "current_dd_r": baseline_stats["max_drawdown_r"],
            "notes": "",
        }

        if baseline_stats["trades"] < SD24_BASELINE_REF["trades"]:
            parity["status"] = "FAIL"
            parity["notes"] = "trade count below frozen baseline"
        elif baseline_stats["trades"] == SD24_BASELINE_REF["trades"]:
            problems = []
            if abs(baseline_stats["profit_factor"] - SD24_BASELINE_REF["pf"]) > 0.001:
                problems.append("PF drift")
            if abs(baseline_stats["total_r"] - SD24_BASELINE_REF["r"]) > 0.05:
                problems.append("R drift")
            if abs(baseline_stats["max_drawdown_r"] - SD24_BASELINE_REF["dd_r"]) > 0.05:
                problems.append("DD drift")
            if problems:
                parity["status"] = "FAIL"
                parity["notes"] = "|".join(problems)
        else:
            parity["status"] = "PASS_NEWER_TRADES"
            parity["notes"] = "additional newer baseline trades present"

        write_csv(
            SD24_OUT["baseline_parity"],
            [parity],
        )

        if parity["status"] == "FAIL":
            raise RuntimeError(
                f"Frozen #24 baseline parity failure: {parity}"
            )
        ...
        SD24_STATUS.update(
            state="standalone_confirmation",
            message="Running predeclared sweep-depth stability matrix",
            progress=20,
        )

        for n, depth in enumerate(SD24_ALL_DEPTHS, 1):
            a_ix = sd24_a_indices_at_depth(
                base_a,
                sweep_depth,
                depth,
            )
            ...
            trades = sd24_build_trades(
                ...
                base_b,
                cost_pips=1.0,
                candidate_id=f"EURJPY_M15_SHORT_DEPTH_{depth:.3f}",
            )
            depth_trades[depth] = trades

            manifest_rows.append({
                "sweep_depth_atr": depth,
                "role": (
                    "PREDECLARED_CANDIDATE"
                    if abs(depth - SD24_CANDIDATE_DEPTH) < 1e-12
                    else "DIAGNOSTIC_NEIGHBOUR_ONLY"
                    if depth in SD24_DIAGNOSTIC_DEPTHS
                    else "CONTEXT_ANCHOR"
                ),
                "raw_a_control_signals": len(base_a),
                "raw_a_signals_after_depth": len(a_ix),
                "a_signal_retention_pct": (
                    100.0 * len(a_ix) / len(base_a)
                    if base_a else 0.0
                ),
                "raw_b_signals_frozen": len(base_b),
                "a_rr": Q24_A_RR,
                "b_rr": Q24_B_RR,
                "stop_ticks": Q24_STOP_TICKS,
                "primary_cost_pips": 1.0,
                "same_candle_priority": "B",
                "candidate_selection_locked_to_0p10": True,
            })

            period_rows.extend(
                sd24_standalone_period_rows(
                    depth,
                    trades,
                )
            )
            ...
            # Cost stress across the local neighbourhood + 0.05/0.20 anchors.
            if depth in [0.05, 0.075, 0.10, 0.125, 0.15, 0.20]:
                cost_rows.extend(
                    sd24_cost_rows(
                        depth,
                        eurjpy_m15,
            ...
            SD24_STATUS.update(
                state="standalone_confirmation",
                message=f"Sweep-depth setting {n}/{len(SD24_ALL_DEPTHS)}",
                progress=20 + int(35 * n / len(SD24_ALL_DEPTHS)),
            )

        standalone_summary = sd24_standalone_summary(
            period_rows
        )
        rolling_summary = sd24_rolling_summary(
            rolling_rows
        )
        calendar_summary = sd24_calendar_summary(
            calendar_rows
        )
        neighbour_rows = sd24_neighbour_check(
            standalone_summary,
            rolling_summary,
            calendar_summary,
        )
        ...
        write_csv(
            SD24_OUT["depth_manifest"],
            manifest_rows,
        )
        write_csv(
            SD24_OUT["standalone_summary"],
            standalone_summary,
        )
        write_csv(
            SD24_OUT["standalone_periods"],
            period_rows,
        )
        write_csv(
            SD24_OUT["cost_stress"],
            cost_rows,
        )
        write_csv(
            SD24_OUT["rolling"],
        ...
        write_csv(
            SD24_OUT["calendar_summary"],
            calendar_summary,
        )
        write_csv(
            SD24_OUT["candidate_neighbour_check"],
            neighbour_rows,
        )

        # Sanity check that fixed 0.10 remains near the already-observed result.
        cand010 = next(
            x for x in standalone_summary
            if abs(x["sweep_depth_atr"] - 0.10) < 1e-12
        )

        if cand010["full_trades"] < 100:
            raise RuntimeError(
                f"0.10 candidate unexpectedly sparse: {cand010}"
            )

        # ------------------------------------------------------------
        # EXACT PORTFOLIO ADD — FIXED 0.10 CANDIDATE AT FULL 1%
        # ------------------------------------------------------------
        SD24_STATUS.update(
            state="portfolio_add",
            message="Running exact current23 -> filtered #24 portfolio add at 1%",
            progress=60,
        )

        candidate010_raw = depth_trades[0.10]
        original_raw = baseline_trades

        portfolio_summary_rows = []
        portfolio_gate_rows = []
        portfolio_displacement_rows = []
        ...
        for priority, mode in [
            ...
            # Current23 exact 1%-each baseline.
            risk23 = {
                sid: 0.01
                for sid in {
                    t["strategy_id"]
            ...
            # Original #24 at full 1% for direct comparison.
            orig_independent = sorted(
                current23["independent"] + original_raw,
                key=lambda t: (
                    t["entry_time"],
                    t["strategy_id"],
                ),
            )
            orig_accepted, orig_rejected = apply_live_safe_nonhedging_gate(
                orig_independent,
                priority,
            )
            risk_orig = {
                ...
                for sid in {
                    t["strategy_id"]
                    for t in orig_accepted
                }
            }
            sim_orig = w24_simulate_equity(
                orig_accepted,
                risk_orig,
                STARTING_BALANCE,
            )

            # Fixed 0.10 replacement #24 at full 1%.
            cand_independent = sorted(
                current23["independent"] + candidate010_raw,
                key=lambda t: (
                    t["entry_time"],
                    t["strategy_id"],
                ),
            )
            cand_accepted, cand_rejected = apply_live_safe_nonhedging_gate(
                cand_independent,
                priority,
            )
            risk_cand = {
                ...
                for sid in {
                    t["strategy_id"]
                    for t in cand_accepted
                }
            }
            sim_cand = w24_simulate_equity(
                cand_accepted,
                risk_cand,
                STARTING_BALANCE,
            )
            ...
            variants = [
                ("CURRENT23_BASELINE", baseline23, sim23),
                ("ORIGINAL_Q24_AT_1PCT", orig_accepted, sim_orig),
                ("SWEEP_0P10_Q24_AT_1PCT", cand_accepted, sim_cand),
            ]

            for variant, trades, sim in variants:
                portfolio_summary_rows.append(
                    sd24_portfolio_summary_row(
                        mode,
                        variant,
                        trades,
                        sim,
            ...
            orig_acc_q24 = [
                t for t in orig_accepted
                if t["strategy_id"] == Q24_STRATEGY_ID
            ]
            cand_acc_q24 = [
                t for t in cand_accepted
                if t["strategy_id"] == Q24_STRATEGY_ID
            ]

            portfolio_gate_rows.extend([
                {
                    "portfolio_mode": mode,
                    "variant": "ORIGINAL_Q24_AT_1PCT",
                    "raw_q24_trades": len(original_raw),
                    "accepted_q24_trades": len(orig_acc_q24),
                    "rejected_q24_trades": (
                        len(original_raw) - len(orig_acc_q24)
                    ),
                    "accepted_portfolio_trades": len(orig_accepted),
                },
                {
                    "portfolio_mode": mode,
                    "variant": "SWEEP_0P10_Q24_AT_1PCT",
                    "raw_q24_trades": len(candidate010_raw),
                    "accepted_q24_trades": len(cand_acc_q24),
                    "rejected_q24_trades": (
                        len(candidate010_raw) - len(cand_acc_q24)
                    ),
                    "accepted_portfolio_trades": len(cand_accepted),
                },
            ])

            portfolio_displacement_rows.extend(
                sd24_displacement_rows(
                    mode,
                    baseline23,
                    cand_accepted,
                    cand_rejected,
        ...
        portfolio_rolling_summary = sd24_portfolio_rolling_summary(
            portfolio_rolling_rows
        )
        portfolio_calendar_summary = sd24_portfolio_calendar_summary(
            portfolio_calendar_rows
        )

        # Deltas against BOTH current23 and original #24 at 1%.
        summary_lookup = {
            (x["portfolio_mode"], x["variant"]): x
        ...
        for mode in [
            ...
            current = summary_lookup[
                (mode, "CURRENT23_BASELINE")
            ]
            original = summary_lookup[
                (mode, "ORIGINAL_Q24_AT_1PCT")
            ]
            candidate = summary_lookup[
                (mode, "SWEEP_0P10_Q24_AT_1PCT")
            ]

            for comparison_name, ref in [
                ("VS_CURRENT23", current),
                ("VS_ORIGINAL_Q24_1PCT", original),
            ]:
                portfolio_delta_rows.append({
                    "portfolio_mode": mode,
                    "candidate": "SWEEP_0P10_Q24_AT_1PCT",
                    "comparison": comparison_name,
                    "reference_variant": ref["variant"],
                    "delta_accepted_trades": (
                        candidate["accepted_portfolio_trades"]
                        - ref["accepted_portfolio_trades"]
                    ),
                    "delta_cagr_pp": (
                        candidate["historical_cagr_pct"]
        ...
        write_csv(
            SD24_OUT["portfolio_summary"],
            portfolio_summary_rows,
        )
        write_csv(
            SD24_OUT["portfolio_delta"],
            portfolio_delta_rows,
        )
        write_csv(
            SD24_OUT["portfolio_gate_summary"],
            portfolio_gate_rows,
        )
        write_csv(
            SD24_OUT["portfolio_gate_displacements"],
            portfolio_displacement_rows,
        )
        write_csv(
            SD24_OUT["portfolio_rolling"],
            portfolio_rolling_rows,
        )
        write_csv(
            SD24_OUT["portfolio_rolling_summary"],
            portfolio_rolling_summary,
        )
        write_csv(
            SD24_OUT["portfolio_calendar"],
            portfolio_calendar_rows,
        )
        write_csv(
            SD24_OUT["portfolio_calendar_summary"],
            portfolio_calendar_summary,
        )
        write_csv(
            SD24_OUT["portfolio_periods"],
            portfolio_period_rows,
        )
            ...
            [
                {
                    "item": "candidate_lock",
                    "value": (
                        "0.10 ATR minimum Trigger-A sweep depth is fixed "
                        "before this confirmation. Neighbour results must "
                        "not be used to switch the candidate to 0.075, "
                        "0.125 or 0.15 simply because one backtests better."
                    ),
                },
                {
                    "item": "neighbour_scope",
                    "value": (
                        "0.075 / 0.125 / 0.15 are local stability checks. "
                        "0.00 / 0.05 / 0.20 are context anchors only."
                    ),
                },
                {
                    "item": "frozen_rules",
                    "value": (
                        "Trigger B, A/B RR 4.75/3.00, B same-candle priority, "
                        "10-tick stop buffer, p0 half-open overlap and "
                        "exit-candle re-entry remain frozen."
                    ),
                },
                {
                    "item": "cost_stress",
                    "value": (
                        "Standalone local neighbourhood is stress-tested at "
                        "1/2/3 adverse pips without changing signals."
                    ),
                },
                {
                    "item": "portfolio_test",
                    "value": (
                        "Only the predeclared 0.10 candidate is carried into "
                        "the exact current23 -> current24 portfolio-add test, "
                        "at full 1.00% risk."
                    ),
                },
                {
                    "item": "portfolio_comparators",
                    "value": (
                        "Portfolio output contains current23 baseline, the "
                        "original unfiltered #24 at 1%, and the 0.10 sweep "
                        "replacement at 1% under both H1-first and M15-first "
                        "live-safe gate orderings."
                    ),
                },
                {
                    "item": "decision_principle",
                    "value": (
                        "A credible replacement should show a stable local "
                        "neighbourhood, retain recent/rolling/cost robustness, "
                        "and materially improve the 1% portfolio drawdown "
                        "trade-off versus the original #24."
                    ),
                },
                {
                    "item": "historical_not_forecast",
                    "value": (
                        "All CAGR, PF and drawdown figures are historical "
                        "backtest outputs, not forecasts."
                    ),
                },
            ],
        ...
        SD24_STATUS.update(
            state="packaging",
            message="Packaging sweep-depth confirmation and portfolio-add results",
            progress=96,
        )
        ...
        SD24_STATUS.update(
            state="complete",
            message=(
                "EURJPY M15 SHORT #24 0.10 sweep-depth confirmation "
                "and 1% portfolio-add test complete"
            ),
            progress=100,
            results=SD24_BUNDLE,
            fixed_candidate_sweep_depth_atr=SD24_CANDIDATE_DEPTH,
            neighbour_depths=SD24_DIAGNOSTIC_DEPTHS,
        )
        ...
        print(
            "SWEEP DEPTH CONFIRMATION ERROR:",
            repr(e),
            flush=True,
        )
...
@app.route("/eurjpy-m15-short-24-sweep-confirm/status")
def sd24_status():
    return jsonify(SD24_STATUS)


@app.route("/eurjpy-m15-short-24-sweep-confirm/results")
def sd24_results():
    if not os.path.exists(SD24_BUNDLE):
        return jsonify({
            "status": "not_ready",
            "state": SD24_STATUS.get("state"),
            "message": SD24_STATUS.get("message"),
        }), 404

    return send_file(
        os.path.abspath(SD24_BUNDLE),
        as_attachment=True,
        download_name=SD24_BUNDLE,
    )


@app.route("/eurjpy-m15-short-24-sweep-confirm/info")
def sd24_info():
    return jsonify({
        "service": (
            "EURJPY M15 SHORT #24 0.10 ATR sweep-depth confirmation "
            "plus exact 23->24 portfolio add"
        ),
        "read_only": True,
        "orders_supported": False,
        "fixed_candidate_sweep_depth_atr": 0.10,
        "diagnostic_neighbours_atr": [0.075, 0.125, 0.15],
        "context_anchors_atr": [0.0, 0.05, 0.20],
        "candidate_portfolio_risk_pct": 1.00,
        "candidate_selection_locked": True,
        "routes": [
            "/eurjpy-m15-short-24-sweep-confirm/status",
            "/eurjpy-m15-short-24-sweep-confirm/results",
            "/eurjpy-m15-short-24-sweep-confirm/info",
        ],
    })
